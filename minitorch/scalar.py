...
def central_difference(f, *vals, arg=0, epsilon=1e-6):
    ...
    lhs_arg = tuple(vals[i] if i != arg else vals[i] + epsilon for i in range(len(vals)))

    rhs_arg = tuple(vals[i] if i != arg else vals[i] - epsilon for i in range(len(vals)))

    return (f(*lhs_arg) - f(*rhs_arg))/(2*epsilon)
## Task 1.2 and 1.4
...
def derivative_check(f, *scalars):
    ...
    for i, x in enumerate(scalars):
        ...
        print(5.5 + x.data, x.derivative, check)
